...
- ココフォリアインポートとActiveEffectも手動確認済みで、**移行タスクはすべて完了**
- `as any` は55箇所（Phase 2着手時の実測）→ リファクタリングPhaseで0まで削減し、lintで禁止した
...
- [x] 主要動作確認: 技能判定・共鳴判定・シートのPlay/Edit切替・ココフォリアインポート・ActiveEffect（すべてv14実機で確認済み）
