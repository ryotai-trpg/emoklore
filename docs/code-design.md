...
- **この表は機械が見ている**。方向は `npm run check:layers` がこの表を仕様として読んで検査する（チェッカー側に規約の写しは無いので、行を書き換えれば検査もそのまま変わる）。循環はlintが落とす（`noImportCycles`）

現状の例外は1つで、[アーキテクチャ](/architecture) の「既知の構造的課題」に記録してある。`config/index.ts` が事前ローカライズの登録のために `utils/` をimportしている。`check:layers` のallowlistに載っているのはこの1件だけになる。
...
名前空間はPascalCaseで切る。**ドットを含むキーを1本の文字列で書かない** — 本体は読めるが、木として辿れなくなる。**`EMOKLORE` の直下に裸のリーフを置かない**（名前空間と同じ列に文字列が並ぶと、どちらなのかが読めない）。この2つは `npm run check:lang` が機械で見ている。

リーフの綴りは**そのキーがコード側の何かを写しているか**で決まる。

| | 綴り | 例 |
|---|---|---|
| コード側のキーを写すもの | camelCase | `Config.*`（configの表のキー）、`*.FIELDS.*`（スキーマ）、`Status.*` `Result.*` `Category.*` `TargetGroup.*` `Aspect.*` `Phase.*`（`choices` の値）、`Sheet.character.tab.*`（タブid）、`Sheet.class.*`（種別id）、`Format.*` |
| 画面に出す文字列 | PascalCase | `ApplyDamage.Title` `Import.Success` `SkillRequest.Skills` `ChatMessage.weapon.Attack` |

**写しているものは綴りを変えない。** 変えるとキーどうしの対応が目で追えなくなり、`check:schema-doc` のような突き合わせも成立しなくなる。逆に、画面の文字列を camelCase で置くと、隣の `Config.*` と見分けが付かない。
...
**機械で守れるものは `npm run check:lang` に入れてある。** ここには規約そのものを書く。

| | 決め |
|---|---|
| ディーラー役の呼称 | 文として読ませるところ（通知・設定の説明・カードの本文）は**「ディーラー」**、字数を詰めたい1行ヒントやラベルは**「DL」**。**「GM」は使わない** — 本体のユーザー設定に出る役割名が「ディーラー」なので、読み替えを強いることになる |
| 文末 | 日本語は **`。` で終える**。本体の日本語UI（FoundryVTTja）が同じ形なので、シートとその外で手触りが変わらない。ラベル・選択肢・ボタン・プレースホルダは文ではないので付けない |
| 無限記号 | **`∞`（U+221E）**。絵文字の `♾️` は使わない |
| 技能の印（`＊` / `★`） | **訳文に書くのはルールブックの文を引くときだけ。** 技能への参照として名前を出すところは `describeSkillLabel` を通す — 印を付けるかどうかは文脈で変わる（[UI設計の規約](/ui-design)ではなく `module/utils/skill.ts` の `LabelContext` が正）ので、訳文に焼き込むと2箇所で決めることになる |

`check:lang` が見るのは、禁止表記・複文の末尾の句点・**通知に渡しているキーの句点**の3つ。最後のものは `ui.notifications` の呼び出しからキーを拾うので、**変数越しに渡しているキーは見えない**（取り込みの検証エラーがこれ）。そこは人が揃える。
...
- **閉じたIssue・マージ済みPRの番号**。書いてよい番号は2種類だけ。**未解決の制限を追跡するopen Issue**（閉じるときに修正と一緒に消す）と、**主張の証拠がIssueにしか無いもの**（番号は証拠の置き場所への参照であって、経緯の参照ではない）。証拠の参照は **`証拠: #N`** と綴る — `check:issue-refs` が追跡の参照と機械で区別するための印で、閉じたIssueを引いてよいのはこの形だけになる。どちらも、番号を読まなくてもコメント単体で意味が通る文にする
