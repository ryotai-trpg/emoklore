...
**アクセント色はバーの塗りと、効果で動いている値の印（`--emoklore-modified`）にだけ使う**。ホバーの文字色や枠線には使わない。

使う場所を増やすほど「ここが今の主役だ」という信号が薄まる。それに、本体のボタンは独自のホバー色（`--color-warm-2`、赤み）を持っているので、自前の要素だけアクセント色にすると**同じシートの中でホバーの色が2種類できてしまう**。効果の印を例外にしているのは、「普段と違う状態である」ことこそが主役の情報であり、常時は現れない（効果が乗ったときだけ点く）ので信号が薄まらないため。
...
同じ規則が2箇所以上に書かれていたら寄せる。寄せ先は**2つあり、層をまたぐかどうかで決まる**。

| | 使う手 | 理由 |
|---|---|---|
| 層をまたいで共有する | `css/components/` の**部品を作り、クラスを合成する** | セレクタを並べる形にすると、`components/` が `applications/` や `chat/` のクラス名を名指しすることになり、**依存が逆向きになる** |
| 1つの層の中で閉じている | **セレクタを並べる**（部品を作らない） | マークアップを1要素1クラスに保てる。`css/applications/actor-sheet.css` がNPCシートと怪異シートの共通分をこの形で持っている |
...
```css
/* 層の中で閉じているもの: セレクタを並べる（css/applications/actor-sheet.css） */
.em-npc__resource,
.em-kai__stat,
.em-kai__field { display: flex; align-items: center; gap: var(--spacer-4); }
```

### 部品にするかの基準

新しく部品を切り出すかは、次の4つが**すべて**当てはまるかで決める。

1. 2箇所以上で繰り返されている
2. その塊が**決めごとを含んでいる**（どの宣言で本体の `--button-size` を外すか、枠線を消して角を丸めるか、など）
3. 片方を変えたらもう片方も変わってほしい
4. **役割で名付けられる**（「ボタン」「画像枠」「札」）。`em-pair`（対を横に並べる）のような**形の名前しか付かないものは部品にしない**
...
`overflow: hidden; text-overflow: ellipsis; white-space: nowrap` は8箇所にあるが部品にしていない。2を満たさない — CSSの定型句で、読めばそのまま意味が分かり、ずれようがない。**数が多いことは理由にならない。**

`em-pair`（ラベルと値を1行に）と `em-num`（桁の少ない数値入力）は一度作って**やめた**。4を満たさず、NPC・怪異の2シートに閉じているのでセレクタを並べれば足りる。戻したことでマークアップはむしろ短くなった（`class` 属性 446→433）。

### この設計の位置づけ

BEMを土台に、層をまたぐ共有だけ合成を許す形になる。[CUBE CSS](https://cube.fyi/) の Composition / Utility / Block に近いが、**Exception（`data-*` で状態を表す）は採っていない** — `data-*` はJSのフック専用という規約を保つため。

「BEMが崩れている」ように見えたら、それは層をまたぐ共有のぶん。1要素に2つ以上のクラスが載るのは37箇所で、内訳は上の表の部品が入っているところになる。
...
- 常時見えるアイコンにするなら `_onRender()` から冪等なメソッドで差す（無ければ `this.window.controls` の前後に生成、あれば更新）。フレームは再renderで作り直されないがヘッダの要素は残るので、状態でアイコンや文言が変わるボタンもここで反映できる（閲覧/編集の切り替えボタンがこれ）
