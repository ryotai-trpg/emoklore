...
**メッセージを書き換えるボタンは、所有者以外には出さない。** 本体は作成者にしか OWNER を返さない（`ChatMessage#getUserLevel`）ので、〔判定〕〔ダメージ〕は他の人が押しても `message.update()` が通らない。効いてくるのは怪異の攻撃カードで、DLが出したものを卓の全員が見る。落とす対象は `OWNER_ACTIONS` の表で、`ACTIONS` と同じくモジュールが足せる。
