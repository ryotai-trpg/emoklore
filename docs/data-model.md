...
ボタンを出す条件の実体は、カードごとの `resolve*CardButtons` が1つだけ持つ。ダメージを振れる条件がカードで違うため（武器は命中していること、怪異はそこに判定なしと空のダメージ式が加わる）。**描画側もモデルの `buttons` もそこを通す** — 別々に書くと片方だけ直したときにずれる。基底の `resolveCardButtons` に既定を置かないのはこのためで、既定があると渡し忘れたカードが黙って別のルールで動く。

`message.rolls` の**添字を読むのは `attackRoll` だけ**にする。怪異の判定なしの攻撃は判定を振らないので `rolls[0]` がダメージになり、他所で添字を書くとダメージを判定として扱う。そのカードは `attackRoll` を上書きして `undefined` を返す。

**メッセージを書き換えるボタンは、所有者以外には出さない。** 本体は作成者にしか OWNER を返さない（`ChatMessage#getUserLevel`）ので、〔判定〕〔ダメージ〕は他の人が押しても `message.update()` が通らない。効いてくるのは怪異の攻撃カードで、DLが出したものを卓の全員が見る。対象は `OWNER_ACTIONS`。`ACTIONS` と違いモジュール向けの拡張点ではない（2枚が同じ1本を共有するうえ、ボタンは保存済みの `content` に焼き込まれた要素なので、名前を足しても描く口が無い）。

落とすのは**描画のたびに走る `addListeners`**であって、テンプレートではない。`content` は作成者のクライアントが描いて保存した1本のHTMLなので、見る人によって出し分けられない。

**ダメージ適用の2つはここに入らない。** あちらはメッセージを書き換えず、権限が足りなければGMへ委譲する（[アーキテクチャ](/architecture)「GMへの委譲」）ので、誰が押しても通る。
...
| `actorUuid` | DocumentUUIDField | `null` | 出した怪異。モジュール連携のための参照 |
